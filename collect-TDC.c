...
unsigned char PTTemp[16];
unsigned char CTTemp[16];
...
unsigned short fileWrite(FILE *fpPT, FILE *fpCT, unsigned short *PT, unsigned short *CT)
    {
	    int i=0;
            for(i=0;i<8;i++)
	    {
		PTTemp[2*i]  =(PT[i] >> 8 ) & 0xff;
		PTTemp[2*i+1]=(PT[i]) & 0xff;

		CTTemp[2*i]  =(CT[i] >> 8 ) & 0xff;
		CTTemp[2*i+1]=(CT[i]) & 0xff;		
	    }
	    for(i=0;i<16;i++)
	    {
		 fprintf (fpPT, "%02X ", PTTemp[i]);
                 fprintf (fpCT, "%02X ", CTTemp[i]);
	    }
	    fprintf (fpPT, "\n");
	    fprintf (fpCT, "\n");
	    fflush(fpPT);	
...
void main()
{
    ...
    strcpy(filenamePT,"text_in-");
    strcpy(filenameCT,"text_out-");
    
    sprintf(tempString, "%d",(tm.tm_year+1900));
    strcpy(filenameDateTime,tempString);
    strcat(filenameDateTime,"-");
    
    //strcat(filenameDateTime,(tm.tm_mon)+"-");
    sprintf(tempString, "%02d",tm.tm_mon);
    strcat(filenameDateTime,tempString);
    strcat(filenameDateTime,"-");
    
    //strcat(filenameDateTime,(tm.tm_mday)+"-");
    sprintf(tempString, "%02d",tm.tm_mday);
    strcat(filenameDateTime,tempString);
    strcat(filenameDateTime,"_");
    
    //strcat(filenameDateTime,(tm.tm_hour)+"_");
    sprintf(tempString, "%02d",tm.tm_hour);
    strcat(filenameDateTime,tempString);
    strcat(filenameDateTime,"-");
    
    //strcat(filenameDateTime,(tm.tm_min)+"_");
    sprintf(tempString, "%02d",tm.tm_min);
    strcat(filenameDateTime,tempString);
    strcat(filenameDateTime,"-");
    
    //strcat(filenameDateTime,(tm.tm_sec)+"");
    sprintf(tempString, "%02d",tm.tm_sec);
    strcat(filenameDateTime,tempString);
    
    strcat(filenameDateTime,".txt");
    
    strcat(filenamePT,filenameDateTime);
    strcat(filenameCT,filenameDateTime);
    
    printf("Filenames: %s \t %s\n", filenamePT, filenameCT);
    
    fpPT = fopen (filenamePT,"w");
    fpCT = fopen (filenameCT,"w");
	...
	for(loop=0;loop<LIMIT;loop++)
	{
		 ...
		 for(i=0;i<8;i++)
		  ...
		  for(i=0;i<8;i++)
	       ...
	       //ct read
	       for(i=0;i<8;i++)
                SASEBOct[i] =SbusRead(ft_handle, 0x0180+i*2);
               

           // calculate AES from software
           // Todo
           ...
           printf("CT: ");
           for(i=0;i<8;i++)
                  printf("%04x ",SASEBOct[i]);
	   printf("\n");
           
           // save Plaintext and Ciphertext
           fileWrite(fpPT, fpCT, pt, SASEBOct);
